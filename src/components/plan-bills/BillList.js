...
function BillList() {
  ...
  <div className='bill-list-container mt-3'>
    {
      // If bill is planed change 'enabled' flag by checkbox
      bills.filter(bill => bill.isPlaned === true).map( (bill, index ) => {
        return (
          <div key={index} className='bill-list-row'>
            ...
            {bill.category}, {bill.title} - <FormattedNumber 
                                              value={bill.price}
                                              style="currency"
                                              currency="UAH" />
            </div>
  ...
  <h6 className='edit edit-mode-btn mt-3' onClick={() =>setEditModeEnabled(true)}>
