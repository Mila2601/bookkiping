...
function Year () {
  ...
  useEffect(() => {
    setPeriod('year');
    // Filtering bills by search value
    if (filtBills.length) {
  ...
  return (<div className='home statistics d-inline-block text-center mt-4 pb-4'>
    <h1>
      <FormattedMessage id="yearStatistic" defaultMessage="Year statistics:" />
    </h1>
        ...
        <input className="w-100 mb-3" id="search" placeholder={typeSearchHere} onChange={e =>
          {
            ...
            // Filter bills by searchValue in category, title or price
            if (searchValue) {
      ...
      <table border="0" cellPadding="0" cellSpacing="0" className="table bg-white">
        <thead>
          <tr className='tr-head'>
            <th data-attr="isPlaned" onClick={
                e => setClick([...click, e.currentTarget.getAttribute('data-attr')])
              }>
            ...
            <th data-attr="category" onClick={
                e => setClick([...click, e.currentTarget.getAttribute('data-attr')])
              }>
              <FormattedMessage id="category" defaultMessage="Category" className="px-2"/>
              <span>&#160;</span>
              <FontAwesomeIcon icon={faArrowUpWideShort} className='up d-none'/>
              <FontAwesomeIcon icon={faArrowDownShortWide} className='down d-none'/>
            </th>
            <th data-attr="title" onClick={
                e => setClick([...click, e.currentTarget.getAttribute('data-attr')])
              }>
